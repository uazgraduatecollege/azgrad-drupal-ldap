...
class LdapUserAdminForm extends ConfigFormBase {
 ...
 before configuring LDAP user. Please go to @link to configure an LDAP server.',
    ...
    $form['manual_drupal_account_editing']['manualAccountConflict'] = array(
      '#type' => 'radios',
      '#options' => array(
        LdapConfiguration::$manualAccountConflictReject => t('Reject manual creation of Drupal accounts that conflict with LDAP Accounts. This only applies to accounts created on user logon;  Account conflicts can still be generated by manually creating users that conflict with ldap users and these users will have their data synced with LDAP data.'),
        LdapConfiguration::$manualAccountConflictLdapAssociate => t('Associate manually created Drupal accounts with related LDAP Account if one exists.'),
        LdapConfiguration::$manualAccountConflictShowOptionOnForm => t('Show option on user create form to determine how account conflict is resolved.'),
      ),
    ...
    $form['basic_to_drupal']['drupalAcctProvisionServer'] = array(
      ...
      '#states' => array(
        // Action to take.
        'enabled' => array(
          ':input[name=drupalAcctProvisionTriggers]' => array('value' => LdapConfiguration::$provisionDrupalUserOnAuthentication),
        ),
      ),
    );
    ...
    $form['basic_to_drupal']['orphanedAccounts']['orphanedCheckQty'] = array(
      '#type' => 'textfield',
      '#size' => 10,
...
EOT;
    ...
    foreach (array('acctCreation', 'userConflictResolve', 'drupalAcctProvisionTriggers', 'mappings__' . LdapConfiguration::$provisioningDirectionToDrupalUser) as $input_name) {
    ...
    foreach (array('orphanedCheckQty', 'orphanedAccountCheckInterval') as $input_name) {
      $form['basic_to_drupal']['orphanedAccounts'][$input_name]['#states']['invisible'] =
        array(
          ':input[name=orphanedDrupalAcctBehavior]' => array('value' => 'ldap_user_orphan_do_not_check'),
        );
    }

    foreach (array('ldapEntryProvisionTriggers', 'password_notes', 'source_drupal_token_notes', 'mappings__' . LdapConfiguration::$provisioningDirectionToLDAPEntry) as $input_name) {
